...
import com.lolinico.technical.open.utils.LocalCache;
...
public abstract class BaseParentActivity<T extends BasePresenter> extends AppCompatActivity implements IHandleMessageCallBack {
    ...
    public T mPresenter;
    Context mContext;
//    boolean isSystemGranted = false;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        Log.d("TAG", "What activity is created? " + getClass().getSimpleName());
        mContext = this;
        doSavedInstanceState(savedInstanceState);

        ActivityManager.getManager().addActivity(this);
        if (getClass().getName().contains("FileCategoryActivity") || getClass().getName().contains("FileChildActivity"))
            LocalCache.getInstance().putPoolActivity(getClass().getName(), this);
    ...
    private static class ActivityHandlerMsgListener extends HandlersManager.SimpleHandlerProxyMsgListener<BaseParentActivity>{
        public ActivityHandlerMsgListener(BaseParentActivity activity) {
            super(activity);
        }
        @Override
    ...
    protected void onDestroy() {
        ...
        if (getClass().getName().contains("FileCategoryActivity") || getClass().getName().contains("FileChildActivity")) {
            Log.d("zeasn", "activitylist:" + LocalCache.getInstance().getPoolActivity());
            LocalCache.getInstance().removePoolActivity(getClass().getName());
        }
